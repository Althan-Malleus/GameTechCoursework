...
void PhysicsEngine::Broadphase() {
	...
	////insert (object, bounding box)
	//BoundingBox box1;
	//BoundingBox box2;

	//CollisionShape* cshapeA;
	//CollisionShape* cshapeB;

	//PhysicsNode* pnodeA;
	//PhysicsNode* pnodeB;

	//float sphereRadius = pnodeA->GetCollisionShape()->GetRadius();
	//float secondSphereRadius = pnodeB->GetCollisionShape()->GetRadius();
...
void PhysicsEngine::buildColPairs() {


	/*build col pairs :-
	bruteforce all current objects
	+ all current objects with parent list
	parentlist += current objs
	...
	if (physicsNodes.size() > 0)
	{
		for (size_t i = 0; i < physicsNodes.size() - 1; ++i)
		{
			for (size_t j = i + 1; j < physicsNodes.size(); ++j)
			{
				...
				{
					if (pnodeA->GetCollisionShape()->GetRadius() + pnodeB->GetCollisionShape()->GetRadius()
			> abs(pnodeA->GetPosition().Length() - pnodeB->GetPosition().Length()))
			...
			if (colDetect.AreColliding(&colData))
			{
				...
				if (okA && okB)
				{
					...
					if (okA && okB)
					{
						...
						{
							//add to list of manidolds that need solving
