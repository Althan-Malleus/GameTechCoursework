...
void Manifold::UpdateConstraint(ContactPoint& c)
{
	...
	/* total velocity going into the collision relative to the collision normal, as elasticity is 'adding' energy back into the system we can attach it to out 'b' term which we already
	add to 'jt' when solving the contact constraint*/

	const float elasticity =
		pnodeA->GetElasticity() * pnodeB->GetElasticity();
	const float elasisty_term = Vector3::Dot(c.colNormal,
	...
	c.b_term += (elasticity * elasisty_term) / contactPoints.size();
}
